...
async def admin_cmd(message: Message):
    if not await is_superadmin(message.from_user.id):
        await message.answer("У вас нет прав для администрирования.")
        return
    await message.answer("Вы в разделе администрирования. Пока функций нет.")
...
# ============ ОБРАБОТЧИК ВСЕХ ПРОЧИХ СООБЩЕНИЙ (ПОИСК) ============
